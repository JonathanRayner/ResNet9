...
In Tensorflow, the default Batch Norm behavior is to initialize the scale (gamma) with ones, so we make no modification to the defaults, but do include the final scalar multiplication layer.
